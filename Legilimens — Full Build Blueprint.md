...
| Capture | Lecture ASR | Transcribes the lecturer in near-real-time, chunks every ~3s, live transcription WebSocket | Faster-Whisper (local, `base.en`, CPU) |
| Capture | Stealth Overlay | Cluely-style transparent, always-on-top frameless window over the teacher's slides (Linux Wayland PipeWire support) | Electron, Next.js |
| Edge buffer | Pi agent & Recording Buffer | Queues pings on Wi-Fi drop; rolling 5-min audio buffer stored to disk with live manifest | Actian Zen, Python deque |
| Embedding | bge-small-en | Turns live transcript chunks into 384-dim vectors immediately (Live Knowledge Base Indexing) | `sentence-transformers`, runs on CPU |
...
| Auth & Storage | User DB | Student and Teacher accounts, persistent ID mapping | MongoDB Atlas, JWT, bcrypt |
...
| Voice | ElevenLabs | Speaks the analogy in a calm tutor voice (Base64 Data URI) | ElevenLabs REST, streaming |
| Dashboard | Marauder's Radar + Pensieve | Live radial heatmap of concept-node confusion + timeline scrubber; post-lecture "worst moments" report, lecture recording review | Next.js, D3.js, Recharts |

...
async def lecture_websocket(websocket: WebSocket, lecture_id: int) -> None:
    ...
    logger.info("WS connect lecture=%d (total=%d)", lecture_id, len(_active_connections[lecture_id]))
    ...
    try:
        ...
        logger.info("WS disconnect lecture=%d (remaining=%d)", lecture_id, len(_active_connections[lecture_id]))
    except Exception:
